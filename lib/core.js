...
function loadSnapshots(dataset, { sinceDate = null } = {}) {
  ...
  for (const f of fs.readdirSync(dir).sort()) {
    ...
    for (const line of fs.readFileSync(path.join(dir, f), 'utf8').split('\n')) {
      ...
      try { rows.push({ _date: d, ...JSON.parse(line) }); } catch (_) {}
    }
  }
  return rows;
}
...
module.exports = { ROOT, DIRS, RATE, stats, today, nowISO, ensureDir, sleep, fetchJSON, appendSnapshot, loadSnapshots, log };
